...
import styles from "./MentionsTextInputStyle";
...
export default class MentionsTextInput extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (!nextProps.inputValue) {
      ...
      nextProps.suggestionsData[this.state.currentTriggerIndex] &&
      nextProps.suggestionsData[this.state.currentTriggerIndex].length !== 0
    ) {
      const numOfRows =
        nextProps.MaxVisibleRowCount >=
        nextProps.suggestionsData[this.state.currentTriggerIndex]
          ? nextProps.suggestionsData[this.state.currentTriggerIndex].length
          : nextProps.MaxVisibleRowCount;
  ...
  updateSuggestions(lastKeyword, trigger) {
    ...
    if (triggerIndex > -1) {
      this.props.triggerCallback(lastKeyword, triggerIndex);
    }
  }
  ...
  onContentSizeChange = event => {
    ...
    this.setState({
      textInputHeight:
        this.props.textInputMinHeight >= contentHeight
    ...
    if (this.state.contentOffset === 0) {
      this.setState({
        textPosition: contentHeight - this.state.contentOffset
      });
    }
  };
  ...
  onScroll = event => {
    ...
    this.setState({
      contentOffset,
      textPosition: this.state.contentHeight - contentOffset
    });
  };
      ...
      <View>
        ...
        <Animated.View
          style={[
            ...
            styles.suggestionsPanelStylePopup,
          ...
          <FlatList
            ...
            renderItem={rowData => {
              return this.props.renderSuggestionsRow[
                this.state.currentTriggerIndex
              ](rowData, this.stopTracking, this._textInput);
            }}
          />
        </Animated.View>
      </View>
...
MentionsTextInput.defaultProps = {
  ...
  suggestionsPanelStyle: { backgroundColor: "rgba(100,100,100,0.9)" },
